...
-- =============================================
-- SecureScan: Migration Complementar (Worker Infrastructure v2)
-- =============================================

-- 1. Adicionar colunas na scan_queue para heartbeat e retry (idempotent)
...
-- 2. Adicionar content_hash na findings para idempotência (idempotent)
...
-- 3. Criar tabela scanner_heartbeats (idempotent)
...
-- 5. RPC: Completar scan (agora com p_page_title)
...
-- 6. RPC: Falhar scan com retry automático
...
-- 9. RPC: Buscar cookies da session (para o worker)
CREATE OR REPLACE FUNCTION public.get_session_cookies(
  p_session_id UUID
) RETURNS TABLE (cookies_encrypted TEXT, url_pattern TEXT) AS $$
BEGIN
  RETURN QUERY
  SELECT ss.cookies_encrypted, ss.url_pattern
  FROM public.scan_sessions ss
  WHERE ss.id = p_session_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;

-- 10. Adicionar quota check na policy INSERT de scans
...
-- 11. Habilitar Realtime na tabela scans (idempotent wrap)
DO $$ BEGIN
